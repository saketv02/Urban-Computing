...
def sirs(g,seeds,beta,gamma,mu):
	tmax=1000
	...
	for t in range(1,tmax):
		...
		if(t>2):
			susceptible_num=int(round(mu*len(R)))
			for i in range(0,susceptible_num):
				node=random.sample(R,1)
				R.discard(node[0])
		...
		for node in set(I):
			...
			for neighbour in neighbours:
				if neighbour not in set(R) and neighbour not in set(I):
					S.add(neighbour)
